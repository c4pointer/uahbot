...
def update(chat_id, name):
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            f"SELECT chat_id from '{table_for_users}' WHERE chat_id=?",
            (chat_id,))
    ...
    try:
        ...
        try:
            if str(call.data).startswith('renew'):
                ...
                active_message_id = call.message.message_id
                command_bank(
                    call, message, call.message.chat.id, active_message_id,
                    language_code)
        except Exception as error:
...
def notification(group_id, text):
    bot.send_message(group_id, text, parse_mode='html')  # Send message to youtube group for reminding
    bot.send_message(
        my_id, 'Notification to Youtube group sent!', reply_to_message_id=topic_youtube)  # To my personal gruop


def schedule_task():
    schedule.every().day.at('12:00').do(current_day)
